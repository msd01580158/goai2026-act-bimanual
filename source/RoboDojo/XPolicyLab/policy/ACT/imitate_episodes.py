...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    # Save best-val-loss checkpoint (deployment via prepare_case loads policy_last.ckpt,
    # so the upload step copies policy_best.ckpt -> policy_last.ckpt).
    if best_ckpt_info is not None:
        best_epoch, min_val_loss, best_state_dict = best_ckpt_info
        best_path = os.path.join(ckpt_dir, "policy_best.ckpt")
        torch.save(best_state_dict, best_path)
        print(f"Best val loss {min_val_loss:.6f} @ epoch{best_epoch + 1}, saved to policy_best.ckpt", flush=True)
