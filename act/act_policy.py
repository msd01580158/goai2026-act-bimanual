...
class ACT:
    ...
    # ── Batch inference (multiple envs in one forward pass) ─────────────────
    # 评测端可能并行跑多个仿真环境，每个环境一个观测，要求一次返回 N 个动作。
    # 实现：N 个观测堆叠 → 一次 policy 前向 → 每环境独立时序聚合（有界缓冲，避免 1.4G/环境）。
    BATCH_MAX_STEPS = 1000  # 有界缓冲（评测 episode 通常 <500 步）

    def reset_batch(self):
        """重置 batch 时序状态（新 episode 开始时调用）。"""
        if hasattr(self, "_env_state"):
            self._env_state.clear()

    def update_obs_batch(self, obs_list):
        self.obs_cache_batch = obs_list
        if not hasattr(self, "_env_state"):
            self._env_state = {}

    def get_action_batch(self):
        obs_list = self.obs_cache_batch
        N = len(obs_list)
        if N == 0:
            return []
        # 堆叠 N 个观测 → batch 前向
        qpos_np = np.stack([np.array(o["qpos"]) for o in obs_list], axis=0)  # (N, dim)
        qpos = torch.from_numpy(self.pre_process(qpos_np)).float().to(self.device)
        imgs = [np.stack([o[c] for c in self.camera_names], axis=0) for o in obs_list]
        curr_image = torch.from_numpy(np.stack(imgs, axis=0)).float().to(self.device)  # (N, ncams, C, H, W)
        with torch.no_grad():
            all_actions = self.policy(qpos, curr_image)  # (N, num_queries, dim)

        BUF = self.BATCH_MAX_STEPS
        actions_out = []
        for i in range(N):
            st = self._env_state.setdefault(i, {"t": 0})
            t = st["t"]
            if "ta" not in st:
                st["ta"] = torch.zeros([BUF, BUF + self.num_queries, self.state_dim]).to(self.device)
            if t % self.query_frequency == 0:
                st["chunk"] = all_actions[i]  # (num_queries, dim)
            if self.temporal_agg:
                ti = t % BUF
                st["ta"][ti, ti:ti + self.num_queries] = st["chunk"]
                col = st["ta"][:, ti]
                populated = col.any(dim=1)
                act = col[populated]
                k = 0.01
                w = np.exp(-k * np.arange(len(act)))
                w = w / w.sum()
                w = torch.from_numpy(w).to(self.device).unsqueeze(1)
                raw = (act * w).sum(dim=0, keepdim=True)
            else:
                raw = st["chunk"][t % self.query_frequency].unsqueeze(0)
            st["t"] = t + 1
            actions_out.append(self.post_process(raw.cpu().numpy()))
        return actions_out